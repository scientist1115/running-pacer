...
- 경로 생성 속도 개선: 후보 경로/채점 API 호출을 순서대로가 아니라 병렬로 처리하고, 느린
  API(횡단보도/노면 체크)는 4~5초 넘으면 그냥 넘어가도록 시간제한을 걸어서 "경로를 준비하고
  있어요"에서 오래 멈춰있던 문제를 줄임
- 러닝이 끝나면(도착 또는 직접 종료) 바로 홈으로 가지 않고, 오늘 뛴 경로 지도와 거리/시간/평균
  페이스, 250m 구간별 페이스 변화 그래프를 보여주는 완료 화면이 먼저 뜸
- 디자인 전면 개편: 이모지 아이콘을 실제 SVG 아이콘으로, 색상/폰트를 신호등(청신호=초록,
  개인마커=앰버) 컨셉으로 통일, 홈 화면에 인사말+목표 진행률 바 추가
- 서비스워커를 "캐시 우선"에서 "네트워크 우선"으로 바꿔서, 새로 배포해도 예전 파일이 계속
  보이던 캐시 버그를 고침
- 나침반(자기센서) 방향을 읽어서, 가만히 서서 몸만 돌려도 러닝 화면 지도가 실시간으로
  같이 회전하도록 함 (iOS는 처음 러닝 시작 버튼 누를 때 센서 권한을 요청함)
- 러닝 화면에 AR 내비게이션 느낌을 더함: 경로 앞쪽으로 행진하는 화살표(쉐브론)를 띄우고,
  경로선/마커에 네온 발광 효과를 주고, 화면 위쪽에 "다음 회전까지 OOm" 배너를 추가함
- 목적지 음성 인식 범위를 넓힘: "OO까지"뿐 아니라 "OO 갈래", "OO로 가자"처럼 말하거나
  그냥 장소 이름만 말해도 목적지로 인식하도록 개선함
- 순서 변경: 이제 "말로 설정" 또는 "직접 입력"으로 목적지를 정하면 먼저 경로를 조회해서
  화면에 그려두고, 화면 아래 "러닝 시작하기" 버튼을 눌러야 그때 3-2-1 카운트다운 후
  실제 GPS 추적이 시작됨 (예전엔 순서가 반대였음)
